(function() {
    ...
    class Particle {
        ...
        draw(ctx) {
            ...
            ctx.fillStyle = this.color + currentOpacity + ')'; ctx.fill();
            if (this.size > 1.5 && currentOpacity > 0.5) {
                ctx.beginPath(); ctx.arc(this.x, this.y, this.size * 2.5, 0, Math.PI*2);
                ctx.fillStyle = this.color + (currentOpacity * 0.2) + ')'; ctx.fill();
            }
        }
    }
    ...
    function animateParticles(timestamp) {
        ...
        gradient.addColorStop(0, 'rgba(0,200,255,0.04)'); gradient.addColorStop(0.5, 'rgba(100,0,200,0.02)'); gradient.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    function createAbortController(forStats = false) {
        if (forStats) {
    ...
    // -------------------- 数据获取 --------------------
    async function fetchAllLogsForRange(startDate, endDate, filterStncode, signal) {
    ...
    function aggregateByStation(data) {
        const map = {};
        (data || []).forEach(row => {
            const body = row.request_body;
            const stncode = body?.stncode;
    ...
    async function loadStats(signal) {
        ...
        document.getElementById('totalCountBack').innerText = allCount || 0;

        document.getElementById('totalCountFront').innerText = totalRows;
        ...
        document.getElementById('todayCount').innerText = cachedTodayCount;
    }
    ...
    async function loadRangeData(silent = false) {
        ...
        try {
            ...
            document.getElementById('dataError').innerHTML = `${escapeHtml('数据加载失败：' + err.message)} <button class="retry-btn" id="retryRangeBtn">重试</button>`;
    ...
    function sortDataInPlace(data, colIndex, ascending) {
        if (colIndex === 8) {
            data.sort((a, b) => {
            ...
            data.sort((a, b) => {
                ...
                switch(colIndex) {
                    ...
                    case 9: valA=bodyA?.questiondata?.length ? bodyA.questiondata[bodyA.questiondata.length-1].answercontent : ''; valB=bodyB?.questiondata?.length ? bodyB.questiondata[bodyB.questiondata.length-1].answercontent : ''; break;
                    default: return 0;
                }
                if (valA < valB) return ascending ? -1 : 1;
                if (valA > valB) return ascending ? 1 : -1;
    ...
    async function updateTodayTasks() {
        ...
        } catch (err) { console.error('获取今日任务失败:', err); tasksEl.innerText = '!'; }
    }
    ...
    function renderSummary() {
        ...
        document.getElementById('summaryInline').innerHTML = items
    ...
    function renderTable(data) {
        ...
        tbody.innerHTML = '';
        if (!data || data.length === 0) { emptyMsg.style.display = 'block'; renderPagination(0); return; }
        ...
        data.forEach(row => {
            ...
            [body?.stncode, body?.shortname, body?.saleno, body?.gname, body?.mobilephone, body?.name,
             body?.ctc, body?.ttc, row.created_at ? row.created_at.substring(0, 19).replace('T', ' ') : '',
    ...
    function onRangeChange() {
        const startEl = document.getElementById('startDateSelect');
        const endEl = document.getElementById('endDateSelect');
    ...
    function buildUI() {
        ...
        <div class="top-bar">
            ...
            <div class="stat-card" id="todayCard" onclick="this.classList.toggle('flipped')">
                <div class="flip-card">
        ...
        ['站点编码','站点简称','销售单号','油品名称','手机号','姓名','CTC','TTC','提交时间','评价内容'].forEach((h, idx) => { const th = document.createElement('th'); th.textContent=h; th.dataset.colIndex=idx; th.addEventListener('click',()=>sortTable(idx)); tr.appendChild(th); });
        ...
        document.getElementById('startDateSelect').addEventListener('change', onRangeChange);
        document.getElementById('endDateSelect').addEventListener('change', onRangeChange);
    ...
    async function showData() {
        ...
        document.getElementById('startDateSelect').value = currentStartDate;
        document.getElementById('endDateSelect').value = currentEndDate;
        ...
        try {
            ...
            progress.hide();
            const errorMsg = `${escapeHtml('系统异常：' + err.message)}`;
    ...
    async function login() {
        ...
        try {
            ...
            hideLoading(); errEl.textContent = '系统异常：' + (err.message || '未知错误');
    ...
    function markStationFormDirty() { stationFormDirty = true; }
